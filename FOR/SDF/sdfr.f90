...
!       Requires a cbn file.
!       I believe this was abandoned in favour of MOLANG
!***********************************************************
!
        use vectors
        ...
        integer natom, nsteps, B(maxAtoms), NbinsR, NbinsT, Rbin, Tbin, theta_bin, phi_bin, Nbinsphi
        integer bonding
        ...
        real(4) angleHist(maxBinsT,maxBinsT,maxBinsT)
...
!             now we look at what the molecule is doing (if it is a molecule)...
              ...
              if (bonding.ne.1) then  ! check to see if the nearest particle is bonded
                ...
                theta_bin = int((o_theta + 180)/(phi_step*(180.0/Pi)) + 1)
 ...
 100    continue
        ! this is the normalization of the speherical volume elements
        do Rbin=1,NbinsR
          do Tbin=1,NbinsT
...
!       these historgrams describe the direction of the other molecules

        open(11,file='angle.0.hist')
        open(12,file='angle.1.hist')
        open(13,file='angle.2.hist')

        do i=1,Nbinsphi
          do j=1,Nbinsphi*2
              write(11,*) (i-0.5)*phi_step-90, j, angleHist(i,j,1)   ! these indicies mean N,NE,E
              write(12,*) (i-0.5)*phi_step-90, j, angleHist(i,j,15)
              write(13,*) (i-0.5)*phi_step-90, j, angleHist(i,j,30)
          enddo
        enddo
