    ...
    if (B(j).ne.0) then 
      ...
      do k=1,natom
        ...
        if (k.ne.j.and.k.ne.B(j)) then
          ...
          all_theta = acos( (px*oox + py*ooy + pz*ooz)/(dop*do_oo) )  ! radians
...
!          endif
      ...
      R = ndo
      bearing = nto    ! radians
                       ! this bearing points towards nearest particle
      ...
      if (bonding.ne.1) then  ! check to see if the nearest particle is bonded
        ...
        bearing = acos(dot_product(bvec,avec)/(length(bvec)*length(avec)))  ! I think this deals with the length?
